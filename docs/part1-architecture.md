...
Use Zephyr; keep a portable no-alloc sensor manager with sequence numbers and drop accounting; 
make BLE a pure consumer that batches to the negotiated MTU. 
On hardware, isolate PPG power and buses, design the antenna for a worn finger. 
Spend power budget on the controls that matter (PPG duty, BLE wake rate)
Expand sensors through a discoverable capability table. 
Treat every µA number as a hypothesis until measured.
...
| Board variants | Devicetree / Kconfig — new sensors as overlays |
| Host testing | `native_sim` + portable core → CI without hardware |
| Updates | MCUboot + SMP/mcumgr → signed DFU over BLE |

**Why not bare-metal** six mixed-rate streams, BLE timing, offline log, and DFU are an RTOS-shaped problem. Power will be dominated by radio and PPG LEDs.
...
### 1.3 Dual-core (PPR). Measure first.
...
**PPR cost:** shared memory protocol, cross-core locking, dual firmware images.

**Decision rule** first EVK power measurement of PPR vs DMA-only. 
DMA may deliver most of the energy win at far less complexity.
...
Ring buffers are the only shared mutable state. 
Critical sections stay short: index math + one bulk `memcpy`, not per-record work under the lock. 
Target worst-case hold well under BLE timing margins (check with a GPIO toggle on a logic analyzer).
...
### Antenna
A finger is lossy and detunes resonance.

- Hard keep-out: no battery, pour, or motor under the antenna
- Tune on a **finger phantom / worn fixture**
...
50 Hz IMU is only ~600 B/s - easy for 2M PHY. 
The expensive part is **radio wakeups**.
...
**Research mode misses 24 h by ~5% on this estimate.** 
Fix candidates: lower PPG rate, gate gyro to motion, 50 ms conn. Do not claim 24 h until measured.
...
### 4.4 Mechanisms
Tickless Zephyr PM; 
deep sleep + RAM retention between BLE events; 
runtime PM + load switches; interrupt-driven wake; align batch work to connection events.
...
**Trade** 
data density for runtime.  
Cell size is mostly fixed by mechanics; **duty cycle** (especially PPG) is the lever.  
...
| A2 | Current table is estimates; | Any line can move a lot, PPG especially |
...
2. **Real PPG energy at usable SNR** LED current × width × rate on real fingers / skin tones vs reference  
